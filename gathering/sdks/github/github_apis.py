...
def get_userinfo_list(username_list, token):
    ...
    for i, username in enumerate(username_list):
        ...
        r = requests.get(url='%s' % url,
                         headers={'Authorization': 'token %s' % token})
        r_body = json.loads(r.text)
        bio = r_body['bio']
        if bio is not None:
            bio = bio.replace('\n', ' ')
        userinfo_list.append({'username': username, 'location': r_body['location'], 'bio': bio})
...
def create_word_cloud(userinfo_list, image_file_name, target='bio'):
    ...
    stop_words = [u'am', u'is', u'of', u'and', u'the', u'to', u'it', u'for', u'in', u'as', u'or', u'are', u'be',
                  u'this', u'that', u'will', u'there', u'was']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("  Using: python main.py 'Repository Name' 'OAuth Token'\n"
    ...
    create_word_cloud(file_to_json(USERINFO_JSON_FILE), 'word_cloud_%s.png' % now, target='bio')
